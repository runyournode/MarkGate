...
def background_update_task(
    file_hash: str, version: str, filename: str, content: bytes, content_type: str
) -> None:
    try:
        ...
        try:
            ...
            if filename not in aliases.filenames:
                ...
                s3_client.put_object(
                    Bucket=S3_BUCKET, Key=alias_key, Body=aliases.model_dump_json()
                )
        except ClientError:
            new_aliases = GlobalFileAliases(file_hash=file_hash, filenames=[filename])
            s3_client.put_object(
                Bucket=S3_BUCKET, Key=alias_key, Body=new_aliases.model_dump_json()
            )
        ...
        s3_client.put_object(
            Bucket=S3_BUCKET, Key=meta_key, Body=meta.model_dump_json()
        )
...
async def call_upstream_backend(
    url: str, content: bytes, headers: dict[str, str], params: dict[str, str]
) -> dict:
